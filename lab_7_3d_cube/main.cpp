...
double rotate_by_x[4][4];
double rotate_by_y[4][4];
double rotate_by_z[4][4];

static void build_x_rotation_matrix(){

}

static void build_y_rotation_matrix(){
    rotate_by_y[0][0]=cos(theta);
    rotate_by_y[0][1]=0;
    rotate_by_y[0][2]=sin(theta);
    rotate_by_y[0][3]=-(local_origin.x * cos(theta))-(local_origin.z*sin(theta))+local_origin.x;
    ...
    rotate_by_y[2][0]=-sin(theta);
    rotate_by_y[2][1]=0;
    rotate_by_y[2][2]=cos(theta);
    rotate_by_y[2][3]=(local_origin.x * sin(theta))-(local_origin.z*cos(theta))+local_origin.z;
...
static void build_z_rotation_matrix(){
...
void x_rotation(double p[],double new_p[],int n){
    for(int i=0;i<n;i++){
        double sum=0;
        for(int j=0;j<n;j++){
            sum+=rotate_by_x[i][j]*p[j];
        }
        new_p[i]=sum;
    }
}

void y_rotation(double p[],double new_p[],int n){
    for(int i=0;i<n;i++){
        double sum=0;
        for(int j=0;j<n;j++){
            sum+=rotate_by_y[i][j]*p[j];
        }
        new_p[i]=sum;
    }
}

void z_rotation(double p[],double new_p[],int n){
    for(int i=0;i<n;i++){
        double sum=0;
        for(int j=0;j<n;j++){
            sum+=rotate_by_z[i][j]*p[j];
        }
        new_p[i]=sum;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    build_x_rotation_matrix();
    build_y_rotation_matrix();
